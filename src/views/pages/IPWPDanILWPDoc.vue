...
const defaultCategory = ['IPWP', 'ILWP'];

onMounted(() => {
    BerandaService.getBerandas().then((data) => {
        products.value = data; // Initial data, not sorted by datetime
        filteredProducts.value = data.filter((product) => defaultCategory.includes(product.category));
        ...
        const filteredByCategory = data.filter((item) => defaultCategory.includes(item.category));
...
const filterData = () => {
    ...
    filteredData = filteredData.filter((product) => defaultCategory.includes(product.category));
...
const resetDataView = () => {
    ...
    filteredProducts.value = products.value.filter((product) => defaultCategory.includes(product.category));
